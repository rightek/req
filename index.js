...
const send = (endpoint, method, data, headers, events, options) => {
    ...
    return new Promise((resolve, reject) => {
        req.open(method || 'GET', endpoint);

        // set request headers
        if (headers && headers.length > 0) {
            for (let i = 0; i < headers.length; i++) req.setRequestHeader(headers[i].key, headers[i].value);
        }
        ...
        req.onload = () => {
            ...
            if (req.status === 200) {
                ...
                resolve({ response: response || req.response, headers });
            }
            else {
                let response = {
        ...
        req.onreadystatechange = function () {
            // 0
            if (this.readyState == this.UNSENT) options.verbose && console.log('UNSENT');
            // 1
            else if (this.readyState == this.OPENED && events.onOpened) events.onOpened.call();
            // 2
            else if (this.readyState == this.HEADERS_RECEIVED && events.onHeadersReceived) events.onHeadersReceived.call();
            // 3
            else if (this.readyState == this.LOADING && events.onLoading) events.onLoading.call();
            // 4
            else if (this.readyState == this.DONE && events.onDone) events.onDone.call();
        };
    });
}
